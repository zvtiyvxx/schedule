...
async def save_group_handler(message: Message, state: FSMContext):
    ...
    if await check_group_exists(message.text):
        data = await state.get_data()
        group = data['group']
        await save_user_group(user_id, group)
        await message.reply(f"Ваша группа: {group}", reply_markup=button.main_button)
        await state.clear()
    else:
        await message.reply("Такой группы нету", reply_markup=button.main_button)
...
async def notification2(message: types.Message):
    ...
    if msg == "уведомления":
        ...
        await save_notifications_state(user_id, "on")
        await message.reply("Уведомления включены", reply_markup=button.main_button)
    elif msg == "выключить":
        await save_notifications_state(user_id, "off")
